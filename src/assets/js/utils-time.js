 ...
 * @param {string} type str 普通时间格式， utcStr yyyymmddThhmmssZ格式
 */
function getCurrentISOString( type = 'str') {
    const time = {}
    let newTime
    let date = new Date()
    time.y = date.getUTCFullYear()
    ...
    time.min = addZero(date.getUTCMinutes())
    time.s = addZero(date.getUTCSeconds())
    time.ms = addZero(date.getMilliseconds())

    if (type === 'utcStr') {
        newTime = `${time.y}-${time.m}-${time.d}T${time.h}:${time.min}:${time.s}.${time.ms}Z`
    } else {
        newTime = `${time.y}-${time.m}-${time.d} ${time.h}:${time.min}:${time.s}.${time.ms}`
    }
